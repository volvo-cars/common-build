...
describe("Test DependenciesYaml provider", () => {
    ...
    it("Create update for build.yml", async () => {
        ...
        const cbUpdate = updates.dependencyUpdates.find(d => { return d.label === BuildYamlScanner.DEFAULT_TOOL_LABEL })
        if (cbUpdate) {
            ...
            fail(`${BuildYamlScanner.DEFAULT_TOOL_LABEL} update should have been found.`)
        }
        const defaultUpdate = updates.dependencyUpdates.find(d => { return d.label === "download-label" })
        if (defaultUpdate) {
            ...
            fail(`"download-label" update should have been found.`)
        }
