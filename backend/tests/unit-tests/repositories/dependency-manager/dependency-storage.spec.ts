...
import { DependencyStoragImpl } from '../../../../src/repositories/dependency-manager/dependency-storage'
...
describe("Testing raw-model repository", () => {
    ...
    it("Test empty then add then remove", async () => {
        ...
        expect(await service.findDependentSources(ref1)).toEqual([])
        await service.setDependencies(source1, ref1, ref2)
        expect(await service.isKnown(source1)).toEqual([true])
        expect(await service.findDependentSources(ref1)).toEqual([source1])
        expect(await service.findDependentSources(ref2)).toEqual([source1])
